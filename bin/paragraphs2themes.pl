...
use constant FEMINISMDIRECTORY      => './themes/feminism';
use constant LIBERALISMDIRECTORY    => './themes/liberalism';
use constant REPUBLICANISMDIRECTORY => './themes/republicanism';
...
my %themes        = ( 'feminism' => \@feminism, 'liberalism' => \@liberalism, 'republicanism' => \@republicanism );
my %paragraphs    = ();
...
foreach my $theme ( sort keys %themes ) {
	...
	# re-configure
	my $directory = '';
	if    ( $theme eq 'feminism' )      { $directory = FEMINISMDIRECTORY }
	elsif ( $theme eq 'liberalism' )    { $directory = LIBERALISMDIRECTORY }
	elsif ( $theme eq 'republicanism' ) { $directory = REPUBLICANISMDIRECTORY }
	else  { die "Unknown theme ($theme). Call Eric.\n" }
	
	my $keys = $themes{ $theme };
	foreach my $key ( @$keys ) {
