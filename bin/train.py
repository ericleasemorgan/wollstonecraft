...
import glob, os, pickle, sys, numpy as np
...
# get and print sets of positive & negative features, respectively
positive = classifier.feature_log_prob_[1, :].argsort()
print( '; '.join ( np.take( vectorizer.get_feature_names(), positive[ :20 ] ) ), "\n" )
negative = classifier.feature_log_prob_[0, :].argsort()
print( '; '.join ( np.take( vectorizer.get_feature_names(), negative[ :20 ] ) ), "\n" )
